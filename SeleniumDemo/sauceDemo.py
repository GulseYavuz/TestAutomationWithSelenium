...
class SauceTest:
    ...
    def emptyLoginAlert(self):
        ...
        print(f"TEST SONUC: {testResult}")
        sleep(5)


    # username alanın girilip password alanının boş olması ihtimalinde uyarı mesajını gösteren fonksiyon
    ...
    # username ve password alanlarına spesifik bilgiler girildiği zaman verilen uyarı mesajı
    ...
    # uyarı mesajı akabinde çıkan error iconuna tıklandığında bütün error iconlarını kaybeden fonksiyon
    def errorIcon(self):
        driver = webdriver.Chrome(ChromeDriverManager().install())
        driver.get("https://www.saucedemo.com/")    
        driver.maximize_window()
    ...
    # girilen spesifik bilgiler akabinde bizi farklı bir html sayfasına yönlendiren fonksiyon
    ...
    def inventoryList(self):
        ...
        print(f"Toplam Ürün Sayısı: {len(productList)}")
        sleep(100)
